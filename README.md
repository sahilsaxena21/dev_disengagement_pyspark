...
The success of this platform depends on the employees of the organizations that maintain the Github repositories. These employees play a variety of roles, such as attend to code issues, perform code reviews, manage pull requests and perform commits. Maintaining a good work environment for these employees is important to secure the employee’s engagement in a sustainable way.
...
### 1.3 Adaptation From Selected Paper
...
For our analysis, the target variable is a binary one, wheather or not the committer is disengaged or engaged. The time period of importance is the first two years of each employee. After exploration of the data, disengagement is defined as follows: if the number of commits in the last six months of the analysis (i.e. between months 30 to 36), are less than 20% of the commits made in the first six months, then we deem the person to be “possibly_disengaged”, else the person is labelled as “no_signs” (i.e. no signs of disengagement). This stringent criterion (i.e. 20%) is used to account for variations in number of commits by employee due to vacation, sick leave etc. In this way, the intention only the developers with drastic reductions in their commit activity are makred as "possibly_disengaged".
...
## 4.	Results
For our commit activity data, we use our proposed factors to train a classifier to predict whether a developer will be disengaged after he/she performs the first commit in two years. From the univariate analysis in Section 3.3.1, we observed the dataset as being imbalanced, with about 3 times more records labelled as 'no_signs', compared to the label 'disengaged'. Hence for this analysis, We interpret the effectiveness of our model in the context of our research question using the F1-score, due to its robustness o imbalanced data. We use the logistic regression model  and the AUC-ROC score as a basis to interpret the model results . Note that due to technical limitations, only a subset of the features were used for the model, these are: 'no_change_periods',  'tot_repo_comm_absent',  'total_repo_comm_new',  'agg_absent_periods',  'new_ppl_periods' and  'avg_multi_ratio'.
